...
var getsession = function () {
    $.ajax({
        ...
        success: function (response) {
            sids = response[0].UserID;
            //alert(response[0].UserID);
            $('#sessionid').val(response[0].UserID);
            document.getElementById("sessionid").value = response[0].UserID;
            loadData(sids);
        },
...
var loadData = function (sids) {
    var temp = sids;
    var sid = document.getElementById("sessionid").value;
    $.ajax({
        type: "GET",
        url: "/Home/GetSpecififcData?id=" + sids,
        success: onSuccess,
